...
/**
 * An object layer on top of <code>TessDllAPI1</code>, provides character recognition support for common image formats, and multi-page TIFF images beyond the uncompressed,
 ...
 */
public class Tesseract1 extends TessDllAPI1 {
    ...
    public String doOCR(int xsize, int ysize, ByteBuffer buf, Rectangle rect, int bpp) throws TesseractException {
        int resultRead = TessDllBeginPageUprightBPP(xsize, ysize, buf, language, (byte) bpp);
        ...
        if (rect == null || rect.equals(EMPTY_RECTANGLE)) {
            output = TessDllRecognize_all_Words();
        } else {
            // (left, right, top, bottom) specifies a region enclosing the text
            output = TessDllRecognize_a_Block(rect.x, rect.x + rect.width, rect.y, rect.y + rect.height);
        }
